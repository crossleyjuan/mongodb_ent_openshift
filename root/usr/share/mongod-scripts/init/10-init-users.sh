...
function mongo_create_user() {
  ...
  if ! mongo_cmd --host "localhost" --authenticationDatabase admin -u admin -p${MONGODB_ADMIN_PASSWORD} ${1:-} <<<"$js_command"; then
    echo >&2 "=> Failed to create MongoDB user: ${MONGODB_USER}"
    exit 1
  fi
  info "User ${MONGODB_USER} created in ${MONGODB_DATABASE}"
}
...
function mongo_reset_user() {
  if [[ -n "${MONGODB_USER:-}" && -n "${MONGODB_PASSWORD:-}" && -n "${MONGODB_DATABASE:-}" ]]; then
    ...
    if ! mongo_cmd --host localhost --authenticationDatabase admin -u admin -p${MONGODB_ADMIN_PASSWORD} ${MONGODB_DATABASE} <<<"${js_command}"; then
